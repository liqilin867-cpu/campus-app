...
export default function App(){
  ...
  const handleSplit=(sum,pp,ids,names,cb)=>{
    ...
    others.forEach(rm2=>{const m={id:'sn'+Date.now()+rm2,category:'缴费提醒',title:'新分摊待缴',content:cu?.name+'发起了分摊，您需缴 ¥'+pp,time:new Date().toLocaleTimeString().slice(0,5),date:new Date().toLocaleDateString(),unread:true};pu.current.push({name:rm2,data:{messages:[m],billsList:[...s2.map(x=>({...x,status:'待缴费',time:'刚刚（待缴）'})),...(c2||[]).map(x=>({...x,status:'待缴费',time:'刚刚（待缴）'}))]}})});
        ...
        <DialogContent className="sm:max-w-sm">
          ...
          <div className="bg-muted/50 rounded-xl p-4">
            ...
            <form onSubmit={doRecharge}>
              {rt==='校园卡'?<div className="mb-3"><label className="text-[10px] font-semibold text-muted-foreground mb-1.5 block">支付方式</label><div className="grid grid-cols-2 gap-2">{['wechat','alipay'].map(p=><button key={p} type="button" onClick={()=>setRp(p)} className={'py-2.5 rounded-xl text-xs font-bold border transition-all cursor-pointer '+(rp===p?'bg-gradient-to-r from-emerald-500 to-emerald-600 text-white border-emerald-500 shadow-sm':'bg-white dark:bg-gray-800 text-gray-500 dark:text-gray-300 border-gray-200 dark:border-gray-600 hover:border-emerald-300 dark:hover:border-emerald-600 hover:shadow-sm')}>{p==='wechat'?'微信支付':'支付宝'}</button>)}</div></div>
              :<div className="mb-3"><label className="text-[10px] font-semibold text-muted-foreground mb-1.5 block">支付方式</label><div className="grid grid-cols-3 gap-2">{['campus_card','wechat','alipay'].map(p=><button key={p} type="button" onClick={()=>setRp(p)} className={'py-2.5 rounded-xl text-xs font-bold border transition-all cursor-pointer '+(rp===p?'bg-gradient-to-r from-amber-500 to-orange-500 text-white border-amber-500 shadow-sm':'bg-white dark:bg-gray-800 text-gray-500 dark:text-gray-300 border-gray-200 dark:border-gray-600 hover:border-amber-300 dark:hover:border-amber-600 hover:shadow-sm')}>{p==='campus_card'?'校园卡':p==='wechat'?'微信':'支付宝'}</button>)}</div></div>}
              <div><label className="text-[10px] font-semibold text-muted-foreground mb-1.5 block">金额</label><div className="grid grid-cols-4 gap-1.5 mb-2">{[20,50,100,200].map(a=><button key={a} type="button" onClick={()=>setRa(String(a))} className={'py-2 rounded-xl text-xs font-bold border transition-all cursor-pointer '+(ra===String(a)?'bg-blue-600 text-white border-blue-600 shadow-sm':'bg-white dark:bg-gray-800 text-gray-600 dark:text-gray-300 border-gray-200 dark:border-gray-600 hover:border-blue-300 dark:hover:border-blue-500')}>¥{a}</button>)}</div>
              <input type="number" value={ra} onChange={e=>setRa(e.target.value)} className="w-full bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-600 text-sm p-2.5 rounded-xl outline-none focus:border-blue-400 text-center dark:text-gray-100" placeholder="自定义金额" required/></div>
              <button type="submit" className="w-full mt-3 py-2.5 bg-gradient-to-r from-blue-600 to-indigo-600 dark:from-blue-700 dark:to-indigo-800 text-white font-bold text-sm rounded-xl shadow-lg shadow-blue-200 dark:shadow-blue-950 active:scale-[0.98] transition-all cursor-pointer">确认支付 ¥{ra}</button>
            </form></div>
        </DialogContent>
      </Dialog>
